...
class Utils:
    ...
    def _validate_schema(server, trigger_name: str, payload: dict):
        ...
        Schemas = Utils.combine_schema_with_core_schema(schema)
    ...
    def handle_request(server, body: Dict[str, Any], reply: Callable[[bytes], None]):
        ...
        has_error = Utils._validate_schema(
            server,
            trigger_name,
            payload={
                **credentials,
        ...
        def serve(sock):
            ...
            while True:
                ...
                try:
                    ...
                    request_line, json_body, headers = Utils._parse_http_request(
                        data)
                    if request_line[0] != "POST":
                        continue

                    Utils.handle_request(server, json_body or {}, lambda res: conn.send(
                        Utils._format_http_response(res))
                    )
                except Exception as e:
